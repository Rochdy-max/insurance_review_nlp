...
def compute_similarity_scores(texts_a, texts_b, model):
    # Encodage de tous les textes d'un coup
    embeddings_a = model.encode(texts_a, batch_size=64, convert_to_tensor=True, show_progress_bar=False)
    # Encodage de tous les labels d'un coup
    embeddings_b = model.encode(texts_b, batch_size=64, convert_to_tensor=True, show_progress_bar=False)
...
def classify_subjects(texts, model, threshold=0.1):
    ...
    sim_scores = compute_similarity_scores(texts, CANDIDATE_LABELS)

    # On récupère le meilleur label et son score
    best_label_ids = torch.argmax(sim_scores, dim=1).tolist()
    best_scores = torch.max(sim_scores, dim=1).values.tolist()
    ...
    if df is not None:
        if text_col not in df.columns:
            ...
            if "subject" not in df.columns:
                ...
                df['subject'], df['score'] = classify_subjects(df, text_col, sentence_transformer)
...
with tab3:
    ...
    with col_p1:
        ...
        predict_btn = st.button("🔮 Prédire la note", width='stretch', type="primary")
...
with tab4:
    ...
    model_choice = st.radio("Méthode d'analyse",
                             ...
                             horizontal=True)
    analyze_btn = st.button("💬 Analyser le sentiment", width='stretch', type="primary")
...
with tab5:
    st.markdown('<div class="section-title">Recherche des avis</div>', unsafe_allow_html=True)
    ...
    col_f1, col_f2, col_f3, col_f = st.columns(4)
    with col_f1:
    ...
    if input_search:
        sentence_transformer = load_sentence_transformer()
        sim_scores = compute_similarity_scores(df_filtered[text_col], [input_search], sentence_transformer)
        print("Sim Scores", any(sim_scores >= 0.4))
        if any(sim_scores >= 0.4):
            df_filtered = df_filtered.iloc[sim_scores >= 0.4]

    st.caption(f"{len(df_filtered)} avis affichés")

    cols_to_show = [c for c in [text_col, note_col, "sentiment", "subject"]
